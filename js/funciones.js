...
function productosUI(productos, id){
  ...
  for (const producto of productos) {
     $(id).append(`<div  class="card" higth="200px">
                    <img src="${producto._img}" width="200px">
                    <div class="card-body">
                      <h5 class="card-title">${producto.nombre}</h5>
                      <p class="card-text">$${producto.precio}</p>
                      <span class="badge badge-danger">${producto.categoria}</span>
                      <a href="#" id='${producto.id}' class="btn badge-secondary btn-compra">COMPRAR</a>
                    </div>
                  </div>`);
  }
...
function registroCarrito(producto){
  return `<p> ${producto.nombre} 
          ...
          <span class="badge badge-danger"> $ ${producto.subtotal()}</span>
          <a id="${producto.id}" class="btn badge-dark   btn-add">+</a>
          <a id="${producto.id}" class="btn badge-secondary btn-sub">-</a>
